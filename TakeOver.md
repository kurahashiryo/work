...
/ovsdb
　├─library
　│　　OVSDBプロトコルでOVSと通信するためのライブラリ群
　│　　Nettyを使用
　├─ovsdb-ui
　│　　netvirt用dlux拡張モジュール
　│　　dluxにovsdb用の画面を追加できる
　└─southbound
　　　　ODLがサポートするOVSDBの実装(イベント、データスキーマ)

Q5. 知っているfeatureについて利用方法や使い方等 
...
ODLのソースコードを読み解くにはOSGIとkarafの知識は必要不可欠です。まずはOSGIのサンプルプログラムなどをネットで調べてからODLのソースコードを読むことをお勧めします。
